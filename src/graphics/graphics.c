...
static const uint16_t Coffset_y[256] = {
...
static const uint16_t Coffset_x[256] = {
...
static const uint8_t Cshift8[256] = {
...
TTextProperties textProperties;
    ...
    static __at ((48+4)*1024U) uint8_t screen_copy[3*32*64];

    static __at ((48+10)*1024U) uint16_t offset_x[256];
    static __at ((48+10)*1024U+512U) uint16_t offset_y[256]; 
    static __at ((48+11)*1024U) uint8_t  shift8[256];
#else
    static uint8_t screen_copy[3*32*64];

    static uint16_t offset_x[256];
    static uint16_t offset_y[256];
    static uint8_t  shift8[256];
#endif
...
void initCanvas() {
    ...
    memcpy(offset_x, Coffset_x, sizeof(offset_x));
    memcpy(offset_y, Coffset_y, sizeof(offset_y));
    memcpy(shift8, Cshift8, sizeof(shift8));
...
static void writeChar(char cr) {
    ...
    IN_SEGMENT( textProperties.font_segment, PAGE_D, {
		uint8_t c = cr-32;
