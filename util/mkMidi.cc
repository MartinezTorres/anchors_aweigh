...
struct MidiFile {
	...
	struct NoteEvent {
		...
		double toneVolume = 16+velocity/4.; // In AY-3-8910 scale, from 0 quietest, to 15 loudest. 
		double instrumentVolume = 0;
		double controllerVolume = 0;

		double priority = tone/12.; // Higher value means higher priority
		
		double volume() const { return (toneVolume + instrumentVolume + controllerVolume + envelopeVolume)/2; }

		bool operator<(const NoteEvent &rhs) const {
			return (0.125*rhs.priority + rhs.volume()) < (0.125*priority + volume());
		};
...
//			if (frame.size()>2 and frame.count(ne.tone)==0) continue;
        ...
        for (auto &f : frame) printf("%04X %01X ", f.first, int(f.second.amplitude));
        printf("\n");
