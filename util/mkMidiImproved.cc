...
struct AdLibInstrument {
	...
	union {
		...
		struct {
			uint8_t i_decay_rate : 4; // Decay Rate. Determines the diminishing time for the sound. The higher the value, the shorter the decay.
			uint8_t i_attack_rate : 4; // Attack Rate. Determines the rising time for the sound. The higher the value, the faster the attack.
		};
	};
	...
	union {
		...
		struct {
			uint8_t i_release_rate : 4; // Release Rate. Determines the rate at which the sound disappears after KEY-OFF. The higher the value, the shorter the release.
			uint8_t i_sustain_level : 4; // Sustain Level. Determines the point at which the sound ceases to decay and chages to a sound having a constant level. The sustain level is expressed as a fraction of the maximum level. 0 is the softest and F is the loudest sustain level. 
		};
	};
...
struct MidiFile {
	...
	struct NoteEvent {
		...
		NoteEvent(Track &track, int channel, int time_pressed, int tone, int velocity) :
			track(track), channel(channel), time_pressed(time_pressed), tone(tone), velocity(velocity) {}
		
		bool pressed = true;
		
		EnvelopeStatus envelopeStatus = ES_ATTACK; // ADSRD (attack, decay, sustain, release, dead
		// In AY-3-8910 scale, from 0 quietest, to 15 loudest. 
        
        double releaseBaseGain = 0;
        
		double velocityGain = 0; //40*log10(velocity / 127.);
		double envelopeGain = 0;
		double instrumentGain = 0;
		double channelGain = 0;
		double expressionGain = 0;

//		double priority = tone/12.; // Higher value means higher priority
		double priority = 4*tone/4.; // Higher value means higher priority

		int percussionInstrument=0;
		double percussionDuration = 0;
		
		double volume() const { 
            
            AdLibInstrument &instrument = 
				(channel!=9 ? 
					track.program[channel]:
					g_gmPercussionInstruments[percussionInstrument]);
                    
            Log(-1) << (15+std::log2(std::pow(10,(velocityGain + envelopeGain + instrumentGain + channelGain + expressionGain)/10)))
                << " " << envelopeGain
                << " " << instrumentGain
                << " " << channelGain
                << " " << expressionGain
                << " " << instrument.name;
//            return 15+std::log2(std::pow(10,(velocityGain + envelopeGain + instrumentGain + channelGain + expressionGain)/10)); 
            return 15+(velocityGain + envelopeGain + instrumentGain + channelGain + expressionGain)*0.3322; 
        }
		...
		bool process_event(int current_time, std::list<NoteEvent> &notes) {
			...
			if        (status_byte>>5 == 0x04) { // Note off/on;
				...
				if (vv == 0 or ((status_byte>>4) == 0x08)) { // note off
					...
					notes.emplace_back(*this, channel, current_time, kk, vv); 
				}
	...
	std::map<int, FrameActuation> calculateFrame(double fps) {
		...
		for (auto &ne: notes) {
			...
			if (ne.track.controller[ne.channel].count(0x7)) 
				ne.channelGain = 40*log10(ne.track.controller[ne.channel][0x7] / 127.);

			if (ne.track.controller[ne.channel].count(0xB)) 
				ne.expressionGain = 40*log10(ne.track.controller[ne.channel][0xB] / 127.);
			...
			ne.instrumentGain = -instrument.output_level * 0.75;
			
            ne.instrumentGain = 0 ;

			//Log(2) << "ne.instrumentVolume " << ne.instrumentGain << " " << instrument.name << " " << ne.controllerVolume;
            ...
            int ar = instrument.i_attack_rate ^ 0xF;
            int dr = instrument.i_decay_rate ^ 0xF;
            int rr = instrument.i_release_rate ^ 0xF;
            int sl = instrument.i_sustain_level ^ 0xF;

			double attack_time  = attackTimeValuesTable[ar*4+rof]/1000;
			double decay_time  = decayAndReleaseTimeValuesTable[dr*4+rof]/1000.;
			double release_time  = decayAndReleaseTimeValuesTable[rr*4+rof]/1000.;
			
			double attack_rate  = 96./
                attack_time;
			double decay_rate   = 3*(15 - sl)/
                decay_time;
			double release_rate = 96./
                release_time;

            double note_duration = (int(time)-ne.time_pressed)*timeToSeconds;

			//Log(2) << "ne.attack_rate_ " << int(instrument.attack_rate) << " " << instrument.name ;
			//Log(2) << "ne.attack_rate " << attack_rate << " " << instrument.name ;
            
            if (ne.envelopeStatus == ES_ATTACK) {
                
                if (note_duration>attack_time) {
                    ne.envelopeGain = 0;
                    ne.envelopeStatus == ES_DECAY;
                } else {
                    ne.envelopeGain = (attack_time-note_duration) * -attack_rate;
                }
            } else if (ne.envelopeStatus == ES_DECAY) {
                
                ne.envelopeGain = (note_duration-attack_time) * -decay_rate;                
                
                if (instrument.sustain and ne.envelopeGain<3*(15 - sl)) {
                    ne.envelopeGain = 3*(15 - sl);
...
//				ne.priority += 1;
                ...
                double release_duration = (int(time)-ne.time_released)*timeToSeconds;
                ne.envelopeGain = ne.releaseBaseGain + release_duration * -release_rate;                
				
				ne.priority -= 200;
			}

            if(0) if (ne.envelopeStatus != ES_OFF) {
                Log(0) << time << " " << note_duration << " " << ne.envelopeStatus << " " << ne.pressed << " " << instrument.name << " " << ne.percussionDuration;
                Log(1) << attack_time << " " << decay_time << " " << release_time << " " << note_duration;
                Log(2) << attack_rate << " " << decay_rate << " " << release_rate;
                Log(3) << ne.envelopeGain;
            }

			if (not ne.pressed and ne.envelopeStatus < ES_RELEASE) {
                ne.time_released = time;
                ne.releaseBaseGain = ne.envelopeGain;
                ne.envelopeStatus = ES_RELEASE;
            }
			if (ne.envelopeStatus != ES_ATTACK and ne.envelopeGain < -96) ne.envelopeStatus = ES_OFF;
		}
...
//			if (frame.size()>2 and frame.count(ne.tone)==0) continue;
			if (frame.size()>2 and frame.count(ne.tone)==0) {
                if (frame.count(ne.tone+24))
                    ne.priority -= 100;
                if (frame.count(ne.tone+12))
                    ne.priority -= 100;
                if (frame.count(ne.tone+5))
                    ne.priority -= 20;
                
                //ne.priority += 10;
                //ne.envelopeGain = -1000;
                continue;
            }
...
int main(int argc, char *argv[]) {
...

	{
		...
		while (midiFile) {
			...
			for (auto &&fa: F) {
				...
				int note = fa.first;
				for (auto &&cn : noteMap) {
					if (not cn.second.empty() and cn.second.rbegin()->second.note == note) {
			...
			for (auto &&fa : F2) {
				...
				for (auto &&cn : noteMap) {
					if (cn.second.rbegin()->second.volume == 0) {
						...
						cn.second[nFrame].note=note;
                        found = true;
                        //if (cn.first==1) Log(0) << vol;
						break;
					}
				}
                if (not found) Log(0) << "channel not found!";
			}
	...
	{	
		...
		std::string full_name = sourceMidi;
		for (auto &&s : "_!-+'() ")
			for (auto &&c : full_name)
				if (c==s) c = '_';


		std::ofstream oss_H("tmp/inc/"+full_name+".h");
        oss_H << "#pragma once" << std::endl;

		for (size_t j=0; j<data.size(); j++) {
            std::ofstream oss("tmp/"+full_name+"_"+char('a'+j)+".c");
            oss << "// Music file generated from: " << name << ".mid" << std::endl;
            oss << "typedef unsigned char uint8_t;" << std::endl;
			auto &D = data[j];
			oss << "const uint8_t " << name << "_" << char('a'+j) << "[] = {\n\t";
            ...
            oss_H << "extern const uint8_t " << name << "_" << char('a'+j) << "[];" << std::endl;
            oss_H << "USING_MODULE(" << name << "_" << char('a'+j) << ", PAGE_D);" << std::endl;
		}
        oss_H << "extern const AYR " << name << ";"  << std::endl;

		std::ofstream oss_C("tmp/"+full_name+".c");
        oss_C << "#include <common.h>" << std::endl;
        oss_C << "#include <" << full_name << ".h>" << std::endl;
        oss_C << "const AYR " << name << " = { {" <<
            "{" << name << "_a, " << "MODULE_SEGMENT("<< name << "_a, PAGE_D) } , " <<
            "{" << name << "_b, " << "MODULE_SEGMENT("<< name << "_b, PAGE_D) } , " <<
            "{" << name << "_c, " << "MODULE_SEGMENT("<< name << "_c, PAGE_D) }" <<
            "} };" << std::endl;
